...
final class DefaultFxRateLookup
  ...
  @Override public Set<? extends MarketDataId<?>> valueRequirements(FxIndex fxIndex) {
    ...
    final Currency currency = getCurrency().orElse(null);
    if (currency == null || currencyPair.contains(currency)) {
      final FxRateId fxRateId = FxRateId.of(currencyPair, observableSource);
      return ImmutableSet.of(fxRateId);
    } else {
      return ImmutableSet.of(
          FxRateId.of(CurrencyPair.of(currencyPair.getBase(), currency), observableSource),
          FxRateId.of(CurrencyPair.of(currencyPair.getCounter(), currency), observableSource)
      );
    }
  }
